...
public class Faceid2Tests
{
    ...
    public void IsAdmin_with_wrong_email()
    {
        ...
        Assert.False(
            authenticator.IsAdmin(
                new Identity("admin@thecompetition.com", new FacialFeatures("green", 0.9m))
            )
        );
    }
    ...
    public void IsAdmin_with_wrong_face()
    {
        var authenticator = new Authenticator();
        Assert.False(
            authenticator.IsAdmin(new Identity("admin@exerc.ism", new FacialFeatures("blue", 0.9m)))
        );
    }
    ...
    public void Register_new_identity()
    {
        ...
        Assert.True(
            authenticator.Register(
                new Identity("alice@thecompetition.com", new FacialFeatures("blue", 0.9m))
            )
        );
    }
    ...
    public void Register_existing_identity()
    {
        var authenticator = new Authenticator();
        authenticator.Register(
            new Identity("tunde@thecompetition.com", new FacialFeatures("blue", 0.9m))
        );
        Assert.False(
            authenticator.Register(
                new Identity("tunde@thecompetition.com", new FacialFeatures("blue", 0.9m))
    ...
    public void IsRegistered_non_existent_identity()
    {
        ...
        Assert.False(
            authenticator.IsRegistered(
                new Identity("alice@thecompetition.com", new FacialFeatures("blue", 0.8m))
            )
        );
    }
